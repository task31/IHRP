...
### ARCHITECT [ARCHITECT -- Claude Code] -- Phase 14: Resume Redact Format Fix via PyMuPDF _(2026-04-01)_

**Goal:** Replace FPDI (broken for modern PDFs) with PyMuPDF (Python) for format-preserving
resume redaction.

**Mode:** SEQUENTIAL

**Root Cause (confirmed via local test):**
FPDI free parser throws CrossReferenceException on PDFs using compressed cross-reference
streams (PDF 1.5+ object streams). This is common in PDFs generated by Word, Google Docs,
LibreOffice, and many other modern tools -- not limited to Word alone. PyMuPDF handles all
of these natively.

**Test result:**
- Jamieson Soleno PDF (159KB) -> FPDI: CrossReferenceException at setSourceFile
- Same PDF -> PyMuPDF: opens cleanly, text extraction and coordinate detection work correctly
- Python 3.10 + fitz 1.27.2.2 available locally

**Architecture decision:**
- Python script (web/scripts/redact_pdf.py) is the new processing core
- PHP calls it via proc_open() with array args (no shell string -- no injection risk)
- Draft Python script already written during testing -- Cursor should refine, not rewrite
- FPDI + fpdf removed from composer entirely (dead weight)
- If Python not found or script fails -> RuntimeException -> user error (consistent with Phase 13 option A)

**Decisions made:**
- proc_open() array form over exec() -- array args prevent shell injection
- JSON config file as IPC (not argv) -- avoids command line length limits for logo base64
- Keep buildPdf() in service (tested independently as a public method)
- Keep smalot/pdfparser (still used by extractLines for candidate name detection)

**Risks flagged:**
- Python + PyMuPDF must be installed on Bluehost. Need to verify before deploying.
  On most Bluehost Business plans, python3 is available. PyMuPDF may need pip install
  or may already be available. This is a deployment verification step.
- proc_open() must not be disabled in php.ini (Bluehost usually allows it on Business plans)
