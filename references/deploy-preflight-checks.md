...
- **ConsultantController contractUpload uniqueness (added 2026-03-25):** Before the production copy step, verify `web/app/Http/Controllers/ConsultantController.php` contains exactly one `function contractUpload(` declaration. If duplicates are found, abort deploy to prevent `Cannot redeclare ... contractUpload()` fatal errors.
