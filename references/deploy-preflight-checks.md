...
- **API token unblocks UAPI (added 2026-03-24):** With `BLUEHOST_CPANEL_TOKEN` in `.deploy.env`, `VersionControl/retrieve` may return OK — then `python deploy.py --step deploy` is valid. If token is unset or host still returns 403, keep using `ssh-deploy`.
